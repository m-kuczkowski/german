...
export function hydrateRemoteState(
  ...
  const newestChallengeMeta = remoteChallengeTime > localChallengeTime
    ? remoteMeta
    : localMeta;
  const localGrammarSessionTime = new Date(localMeta.activeGrammarSession?.startedAt ?? 0).getTime();
  const remoteGrammarSessionTime = new Date(remoteMeta.activeGrammarSession?.startedAt ?? 0).getTime();
  const newestGrammarSession = remoteGrammarSessionTime > localGrammarSessionTime
    ? remoteMeta.activeGrammarSession
    : localMeta.activeGrammarSession;
  const meta = withMetaDefaults({
    ...
    activeGrammarSession: newestGrammarSession,
...
export function hydrateGrammarProgress(
  localProgress: GrammarTopicProgress[],
  remoteProgress: GrammarTopicProgress[] | undefined,
): GrammarTopicProgress[] {
  const merged = new Map(localProgress.map((item) => [item.topicId, item]));
  for (const remote of remoteProgress ?? []) {
    const local = merged.get(remote.topicId);
    const localTime = new Date(local?.lastPracticedAt ?? 0).getTime();
    const remoteTime = new Date(remote.lastPracticedAt ?? 0).getTime();
    if (!local || remoteTime >= localTime) merged.set(remote.topicId, remote);
  }
  return [...merged.values()].sort((left, right) => left.topicId.localeCompare(right.topicId));
}
